        ...
        for(int bssn_var=0; bssn_var<BSSN_NUM_VARS; bssn_var++){
            ...
            for (int pointInd=0; pointInd<sizeofBlock; pointInd++){
                ...
                if (fabs(diff)>accuracy){
                    ...
                    std::cout <<std::setprecision(NUM_DIGITS)<< std::left << std::setw(numWidth) << std::setfill(separator)  << var_out[bssn_var][blkList[blk].offset+pointInd]+diff;
